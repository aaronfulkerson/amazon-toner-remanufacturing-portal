...
import { LoginErrors, loginSchema } from "@/modules/login";
...
export async function validate(formData: FormData): Promise<SelectUser> {
  ...
  if (!result.success) throw Error(LoginErrors.VALIDATION_FAILED);

  const { data } = result;

  const user = await getUserByEmail(data.email);
  if (!user) throw Error(LoginErrors.INVALID_CREDENTIALS);

  const authenticated = await verifyPasswordHash(
    user.passwordHash,
    data.password
  );
  if (!authenticated) throw Error(LoginErrors.INVALID_CREDENTIALS);
